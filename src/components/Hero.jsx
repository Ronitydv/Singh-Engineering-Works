...
const Hero = ({ onOpenCustomizer }) => {
  ...
  // Track scroll progress of the hero section as it scrolls up/out of view
  const { scrollYProgress } = useScroll({
    target: heroRef,
    offset: ["start start", "end start"]
  })

  // Map progress to drawing path length
  const pathLength = useTransform(scrollYProgress, [0, 1], [0.15, 0.9])

  return (
    <section className="hero-section" id="hero" ref={heroRef}>
      <div className="hero-bg-overlay"></div>
      
      {/* Scroll-drawing line path in background */}
      <LinePath className="hero-scroll-svg" pathLength={pathLength} />
      ...
      <div className="hero-container container">
        ...
        {/* Right Image Column (User provided workshop photo) */}
        <motion.div 
